...
function checkSign(Request $request): bool
{
    // php >= 7.0
    $hashAuthorizationKey = WorldSlotGame::keys()['agentToken']; // '424c65e51942160021fefe9d6d603492'; // Key from back office

    $hashAuth = $request->headers->get('Hash-Authorization') ?? ''; // Get hash from request or specify empty
...
function checkWhiteIP($ip): bool {
  $array = ['35.158.219.10', '3.72.158.132', '65.109.228.159', '2a01:4f9:c012:5e64::1', '252.19.241.70'];
...
Route::get('/nux/sessionCheck', function (Request $request) {
  $hashAuth = $request->headers->get('Hash-Authorization');
  ...
  // dd($userId);
  // Log::info('sessionCheck called '. $ip .'///'. $userId .'///'. $token);
...
Route::get('/nux/playerDetails', function (Request $request) {
  ...
  // dd($userId);
  // Log::info('playerDetails called '. $hashAuth .'////'. $userId .'///'. $token);
...
Route::get('/nux/getBalance', function (Request $request) {
  ...
  // dd($userId);
  // Log::info('getBalance called '. $hashAuth .'////'. $userId .'///'. $token);
  ...
  // Log::info($currency->fiatNumberFormat($currency->convertTokenToEUR($user->balance($currency)->get())));

  $body = [
    'status' => true,
    'balance' => !$user ? 0 : $currency->fiatNumberFormat($currency->convertTokenToEUR($user->balance($currency)->get()))
  ];

  return APIResponse::success($body);
});

Route::get('/nux/getUserToken', function (Request $request) {
  $hashAuth = $request->headers->get('Hash-Authorization');
  $userId = $request->input('userId');

  // dd($userId);
  // Log::info('sessionCheck called '. $hashAuth .'///'. $userId);
  ...
  $body = [
    'status' => true,
    'userId' => $user->_id,
    'token' => $user->getGameToken(),
    'balance' => $currency->fiatNumberFormat($currency->convertTokenToEUR($user->balance($currency)->get()))
  ];

  return APIResponse::success($body);
});

Route::post('nux/moveFunds', function (Request $request) {
  $data = $request->all();
  // Log::info('moveFunds called ');
  // Log::info($data);

  $hashAuth = $request->headers->get('Hash-Authorization');

  // Log::info('moveFunds called------- '. $hashAuth .'////'. $data['userId'] .'///'. $data['gameId'] .'///'. $data['token']);
  ...
  $user = \App\Models\User::where('_id', $data['userId'])->first();
  ...
  if ($user->getGameToken() != $data['token']) {
    $body = [
      'status' => false,
      'errors' => [
        'code' => 417,
  ...
  $game = collect($gameList)->first(function ($game) use ($data) {
      return strval($game['id']) === $data['gameId'];
  });

  // Log::info('moveFunds --game--- ');
  // Log::info($game);

  $currency = \App\Currency\Currency::find($user->selected_currency);

  $body = [
      "round_id" => $data['eventId'], // $data['extraData']['roundId']?$data['extraData']['roundId']:Str::random(10),
      "name" => $game['name'],
      "id" => $game['id'],
      "img" => $game['img'],
      ...
      "transaction" => [
          ...
          "bonusCode" => isset($data['bonusCode'])?$data['bonusCode']:'',
          "direction" => $data['direction'],
          // "user_balance" => $data['user_balance'],
          "bet" => ($data['direction'] == 'debit')?floatval($data['amount']):0,
          "win" => ($data['direction'] == 'credit')?floatval($data['amount']):0,
